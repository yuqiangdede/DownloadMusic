...
def write_apic(mp3_path: Path, cover_jpg: Path) -> bool:
    ...
    try:
        ...
        if parsed:
            ...
            mime = detect_image_mime_from_bytes(data)
        id3.delall("APIC")
        id3.add(
            APIC(
                encoding=3,
...
def extract_valid_image_bytes(data: bytes) -> Optional[Tuple[bytes, str]]:
    if not data:
        return None

    png_sig = b"\x89PNG\r\n\x1a\n"
    pos_png = data.find(png_sig)
    if pos_png >= 0:
        i = pos_png + len(png_sig)
        try:
            while i + 8 <= len(data):
                ...
                if chunk_type == b"IEND":
                    return data[pos_png:end], "image/png"
                i = end
        except Exception:
            pass

    pos_jpeg = data.find(b"\xff\xd8\xff")
    if pos_jpeg < 0:
        pos_jpeg = data.find(b"\xff\xd8")
    if pos_jpeg >= 0:
...
def write_apic_bytes(mp3_path: Path, cover_data: bytes, mime: str) -> bool:
    ...
    try:
        parsed = extract_valid_image_bytes(cover_data)
        if parsed:
...
def sanitize_cover_file(cover_path: Path) -> Path:
    try:
    ...
    try:
        ...
        if target != cover_path and cover_path.exists():
            cover_path.unlink()
        print(
            f"[COVER] 清理封面冗余数据：{target.name} ({len(raw)} -> {len(clean)} bytes)"
        )
        return target
    except Exception:
        return cover_path
...
def try_extract_cover_from_res_ncm(
    ...
    for ncm in candidate_ncms:
        ...
        try:
            ...
            print(
                f"[WARN] 写入 NCM 封面失败：{target} 来源={ncm.name} err={e}",
                file=sys.stderr,
            )
            continue
        if target.exists() and target.stat().st_size > 0:
            print(f"[COVER] 使用 NCM 内嵌封面：{ncm.name}")
            return target, cover_data, mime
...
def prepare_cover_for_dir(
    ...
    if cover_locked and cover_file.exists():
        try:
            size = cover_file.stat().st_size
        except Exception:
            size = 0
        if size <= 1024:
            print(f"[WARN] 目录封面疑似损坏(过小)，删除并重试拉取：{cover_file}")
            try:
                cover_file.unlink()
            except Exception:
                pass
    ...
    cover_file = pick_cover_file(d) or (d / "Cover.jpg")
    if cover_file.exists():
        cover_file = sanitize_cover_file(cover_file)
    ...
    if cover_file.exists():
        ...
        if cover_data:
            patched = 0
            for m in d.rglob("*.mp3"):
                if not m.is_file() or has_apic(m):
                    continue
                if write_apic_bytes(m, cover_data, mime):
                    patched += 1
            if patched:
                print(f"[COVER] 已补写 APIC：{d.name} ({patched} 首)")
...
def gen_mp4_with_cover_jpg(
    ...
    def _prepare_cover_for_encode() -> None:
        ...
        try:
            ...
            if (
                ...
                and is_image_decodable(ffmpeg, target)
            ):
                cover_for_encode = target
                temp_cover_path = target
                print(
            ...
            print(f"[WARN] 预缩放封面异常，将使用原图：{cover_jpg} err={e}", file=sys.stderr)
    ...
    def _build_cmd(codec: str) -> List[str]:
        ...
        return [
            ...
            "1",
            "-i",
            windows_input_path(cover_for_encode),
            "-i",
    ...
    def _cleanup_temp_srt() -> None:
        if temp_srt_path and temp_srt_path.exists() and not dry_run:
            try:
                ...
                pass
        if temp_cover_path and temp_cover_path.exists() and not dry_run:
            try:
